...
public class Graphics
{
    ...
    {
        Vector2d CameraPosition = camera.ToCameraSpace(WorldPosition);
        m_BitmapBox.SetPosition(CameraPosition);
        if(camera.AppearsOnCamera(m_BitmapBox))
        {
            Vector2d ScreenPosition = camera.ToScreenSpace(CameraPosition);
